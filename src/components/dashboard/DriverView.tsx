...
const DRIVERS = [
  ...
  { name:"Elena Rodriguez",emp:"EMP-90554", status:"LEAVE",     statusColor:"bg-[#fef3c7] text-[#d97706]",  vehicle:"Unassigned", license:"Class B", expiry:"Jan 08, 2026", expiryAlert:false, score:4.8, img:"https://i.pravatar.cc/40?img=66" },
];

export default function DriverManagementView({ onNavigate }: { onNavigate?: (p:string)=>void }) {
  ...
  return (
    <Layout activeNav="Driver Management" onNavigate={onNavigate} topbarTitle="Driver Management" searchPlaceholder="Global Search..." userRole="Fleet Manager">
      <div className="p-6 space-y-5 animate-fadein">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label:"On Leave", value:"8", sub:"Scheduled returns this week: 3", icon:"event_busy", color:"text-[#64748b]", bg:"bg-[#f1f5f9]", border:"" },
        ...
        <div className="bg-white rounded-2xl border border-[#e2e8f0] shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[#f1f5f9] flex items-center gap-3">
            ...
            <select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}
              ...
              {["All","AVAILABLE","ON DUTY","OFF DUTY","LEAVE"].map(s=><option key={s} value={s}>Status: {s}</option>)}
            </select>
          ...
          <table className="w-full">
            <thead><tr className="bg-[#f8fafc]">
              {["DRIVER","STATUS","ASSIGNED VEHICLE","LICENSE TYPE","LICENSE EXPIRY","PERFORMANCE","ACTIONS"].map(h=>(
                ...
                <tr key={d.emp} className="border-t border-[#f1f5f9] hover:bg-[#f8fafc] transition-colors group">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      <Icon name="star" className="text-[#f59e0b] text-[16px]" style={{ fontVariationSettings: "'FILL' 1" }} />
                      <span className="text-[13px] font-semibold text-[#0f172a]">{d.score}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3.5">
